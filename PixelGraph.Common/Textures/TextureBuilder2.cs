    ...
    internal class TextureBuilder : ITextureBuilder
    {
        ...
        private bool TryBuildMapping(ResourcePackChannelProperties outputChannel, out TextureChannelMapping mapping)
        {
            mapping = new TextureChannelMapping {
                ...
                OutputPower = outputChannel.Power ?? 1f,
                InvertOutput = outputChannel.Invert ?? false,

                Scale = GetChannelScale(outputChannel.ID),
            };

            var isOutputHeight = EncodingChannel.Is(outputChannel.ID, EncodingChannel.Height);
            var isOutputNormalZ = EncodingChannel.Is(outputChannel.ID, EncodingChannel.NormalZ);
            ...
            if (isOutputHeight) mapping.InvertOutput = true;
            if (isOutputOcclusion) mapping.InvertOutput = true;
            ...
            var src = InputChannels.FirstOrDefault(i
                => EncodingChannel.Is(i.ID, outputChannel.ID));

            if (src?.Texture != null && TryGetSourceFilename(src.Texture, out mapping.SourceFilename)) {
                mapping.ApplyInputChannel(src);

                if (isOutputHeight) mapping.InvertInput = true;
                if (isOutputOcclusion) mapping.InvertInput = true;
                return true;
            }

            // restore normal-z
            if (isOutputNormalZ) {
                mapping.RestoreNormalZ = true;
                return true;
            }
        ...
        private async Task ApplyMappingAsync(TextureChannelMapping mapping, CancellationToken token)
        {
            ...
            try {
                ...
                // TODO: generated fallbacks?
                //if (mapping.RestoreNormalZ)
