...
float4 main(const ps_input input) : SV_TARGET
{
    ...
    for (int i = 0; i < NumLights; i++) {
        ...
        if (Lights[i].iLightType == 1) {
            ...
            float shadow = get_parallax_shadow(shadow_tex, polT, SNoL);
            ...
            if (bHasShadowMap && bRenderShadowMap) {
	            ...
	            if (d > 0) {
					const float4 sp = mul(float4(input.wp.xyz, input.wp.w), mShadowViewProj);
	                shadow *= shadow_strength(sp.xyz / sp.w);
	            }
                else {
	                shadow = 0.0f;
                }
