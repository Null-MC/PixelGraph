...
float4 ComputeSkyInscattering(const in ScatteringParams setting, const in float3 eye, const in float3 V, const in float3 L)
{
	...
	const float phaseNight = 1.0 - saturate(insctrOpticalLength.x * EPSILON);
...
float3 get_sky_color(const in float3 V, const in float3 L)
{
 ...
 //       mouse = float2(0.5, 0.7);
	...
	const float3 eye = float3(0.0f, 1000.0f, 0.0f);
   	return ComputeSkyInscattering(setting, eye, V, L).rgb;
}
